...
public class Prefetcher<K, T> extends AsyncStateClient<K, T> {
    ...
    private transient final ConcurrentHashMap<K, WriteTask<K, T>> pendingWrites;

    // ---------- config ----------
    private final int DRAIN_READS_MAX_OPS = 1, DRAIN_WRITES_MAX_OPS = 1;

    private volatile long lastProcessedTs = 0L; // todo hacky way to make sure prefetching does not cause eviction of things we need
    ...
    private static final class WriteTask<K, T> {
