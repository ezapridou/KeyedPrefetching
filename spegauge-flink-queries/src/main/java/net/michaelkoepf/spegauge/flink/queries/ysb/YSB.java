 ...
 */
public class YSB extends QueryGroupWithConfig {
    ...
    protected List<DataStream> register(StreamExecutionEnvironment env) throws Exception {
        ...
        DataStream<Tuple3<Long, String, Long>> projection = mappedStream
                .map(x -> new Tuple3<Long, String, Long>(x.event.ad_id_long, x.event.adID, x.processingTimeMilliSecondsStart))
                .setParallelism(getParallelism())
                .name("Project")
        ...
        result = projection
                .keyBy(e -> e.f0)
                .process(new JoinRedisTableCache())
                .setParallelism(getParallelism())
                .name("JoinRedis")
